...
def sentence1() -> Expr:
    ...
    cond1 = A | B  # A ∨ B
    cond2 = ~A % (~B | C)  # # ¬A ↔ (¬ B ∨ C)
    cond3 = disjoin(~A, ~B, C)  # ¬ A ∨ ¬ B ∨ C
    
    return conjoin(cond1, cond2, cond3)
    "*** END YOUR CODE HERE ***"
...
def sentence2() -> Expr:
    ...
    return conjoin(cond1, cond2, cond3, cond4)
    "*** END YOUR CODE HERE ***"
...
def entails(premise: Expr, conclusion: Expr) -> bool:
    ...
    """
    "*** BEGIN YOUR CODE HERE ***"
    return False if findModel(~conclusion & premise) else True
...
def atMostOne(literals: List[Expr]) -> Expr:
    ...
    "*** BEGIN YOUR CODE HERE ***"
    print(literals)
    return conjoin([disjoin(~item[0], ~item[1]) for item in itertools.combinations(literals, 2)])
...
def pacphysicsAxioms(t: int, all_coords: List[Tuple], non_outer_wall_coords: List[Tuple], walls_grid: List[List] = None, sensorModel: Callable = None, successorAxioms: Callable = None) -> Expr:
    ...
    "*** BEGIN YOUR CODE HERE ***"
    #If a wall is at (x, y) --> Pacman is not at (x, y)
    pacphysics_sentences.append(conjoin([PropSymbolExpr(wall_str, x, y) >> (~PropSymbolExpr(pacman_str, x, y, time=t)) for (x, y) in all_coords]))
    # Pacman is at exactly one of the squares at timestep t.
...
def checkLocationSatisfiability(x1_y1: Tuple[int, int], x0_y0: Tuple[int, int], action0, action1, problem):
    ...
    # There is no sensorModel because we know everything about the world. 
    # Where needed, use allLegalSuccessorAxioms for transitions since this is for regular Pacman transition rules.
    # Append the pacphysicsAxioms for both time 0 and 1 with the arguments that was said above.
    ...
    # Conjoin the knowledge base with the expressions that was asked and make the two models.
...
def positionLogicPlan(problem) -> List:
    ...
    for t in range(50):
        ...
        # similar to pacphysicsAxioms, but don’t use that method since we are using non_wall_coors when generating the list of possible locations in the first place (and walls_grid later)
        ...
        model = findModel(conjoin(coj_KB, PropSymbolExpr(pacman_str, xg, yg, time=t)))
...
def foodLogicPlan(problem) -> List:
    ...
    "*** BEGIN YOUR CODE HERE ***"
    # Most of the code is the same as the previous question so I'm just copying it here.
    ...
    # Make the expressions and add them to the knowledge base.
    ...
    for t in range(50):
        ...
        # change gx, gy into g and treat every food as (gx, gy)
        g = [~PropSymbolExpr(food_str, x, y, time=t) for (x, y) in food] #goal
        model = findModel(conjoin(conjoin(KB), conjoin(g)))
